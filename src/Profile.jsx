...
const Profile = ({ setSelectedTrack }) => {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                const response = await fetch(`http://3.89.228.54:8080/users/profile/${user.username}`, {
                    headers: {
    ...
    const handleProfileUpdate = async (e) => {
        ...
        try {
            const response = await fetch(`http://3.89.228.54:8080/users/update/${user.username}`, {
                method: 'PUT',
                headers: {
    ...
    const handleProfilePictureChange = async (e) => {
        ...
        if (file) {
            ...
            try {
                const response = await fetch(`http://3.89.228.54:8080/users/profile-picture/${user.username}`, {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`
                    },
